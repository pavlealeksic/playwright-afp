    ...
    await pageOrContext.addInitScript((options) => {
        ...
        const config = {
            random: {
                ...
                number: (power) => {
                    const tmp = [];
                    for (let i = 0; i < power.length; i++) tmp.push(Math.pow(2, power[i]));
                    ...
                    buffer: (target) => {
                        ...
                        proto.bufferData = new Proxy(proto.bufferData, {
                            apply(target, self, args) {
                                ...
                                const noise =
                                    args[1][index] !== undefined ? 0.1 * config.random.value() * args[1][index] : 0;
                            ...
                            apply(target, receiver, args) {
                                if (args[0] === 3415) return 0;
                                    ...
                                    return window.afpOptions.webglData
                                        ...
                                        : config.random.item(["Graphics", "HD Graphics", "Intel(R) HD Graphics"]);
                                else if (args[0] === 7938)
                                    return window.afpOptions.webglData
                            ...
                            for (let i = 0; i < results_1.length; i += 100) {
                                const index = Math.floor(
                                    (window.afpOptions.audioFingerprint
                                        ? window.afpOptions.audioFingerprint.getChannelDataIndexRandom
                                        : Math.random()) * i
                                );
                                results_1[index] =
                                    results_1[index] +
                                    (window.afpOptions.audioFingerprint
                                        ? window.afpOptions.audioFingerprint.getChannelDataResultRandom
                                        : Math.random()) *
                                        0.0000001;
                            }
                        }
                            ...
                            results_2.__proto__.getFloatFrequencyData,
                            {
                                apply(target, self, args) {
                                    ...
                                    for (let i = 0; i < arguments[0].length; i += 100) {
                                        const index = Math.floor(
                                            (window.afpOptions.audioFingerprint
                                                ? window.afpOptions.audioFingerprint.createAnalyserIndexRandom
                                                : Math.random()) * i
                                        );
                                        arguments[0][index] =
                                            arguments[0][index] +
                                            (window.afpOptions.audioFingerprint
                                                ? window.afpOptions.audioFingerprint.createAnalyserResultRandom
                                                : Math.random()) *
                                                0.1;
                                    }
